...
void moveEvenItemsToBack(LinkedList *ll){
	...
	if (ll->size < 2) // check if no need to move
		return;
	cur = ll->head;
	pre = NULL;

	tail = ll->head;
	while(tail->next!=NULL) tail = tail->next; // move to back
	...
	for (i = 0; i < ll->size; i++) {
		if (cur->item % 2 == 0) {
            if(pre==NULL)
               {
                ...
                pre->next = cur->next;
                tail->next = cur;  // even but not first node
                cur->next = NULL;
                cur = pre->next;
                tail = tail->next;
            }
            ...
            pre = cur;   // odd node, to set pre pointer to trigger above else scenario
			cur = cur->next;
		}
	}
}
