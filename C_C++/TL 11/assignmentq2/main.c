...
int SD(Graph g, int v, int z){
    ...
    while(q.size != 0)
    {
        ...
        for(i=0; i<g.V; i++)
        {
            if(g.matrix[getFront(q)-1][i] == 1 && i == z-1)
            {
                if(g.visited[i] == 0)
                {
                    g.visited[i] = g.visited[getFront(q)-1] + 1;
                }
                else if(g.visited[getFront(q)-1] + 1 < g.visited[i])
                {
                    g.visited[i] = g.visited[getFront(q)-1] + 1;
                }
            }
            if(g.matrix[getFront(q)-1][i] == 1 && g.visited[i] == 0)
            {
                g.visited[i] = g.visited[getFront(q)-1] + 1;
                enqueue(&q, i+1);
            }
